...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROListDataLoader alloc] initWithDatasource:[[DatasourceManager sharedInstance] emptyDatasource]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"ll"];

    self.navigationItem.title = NSLocalizedString(@"ll", nil);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Item *item = self.items[(NSUInteger)indexPath.row];
    LlDetailViewController *destinationViewController = [LlDetailViewController new];
